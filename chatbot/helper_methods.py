...
def apply_condition(df, col_name, condition, condition_value):
	if col_name == 'OrderDate' and type(condition_value) == str:
...
def get_date_text(date_condition):
	...
	for date in date_condition:
		text += date_text.get(date['conditions']) + datetime.strftime(date['OrderDate'], '%Y-%m-%d') + ' & '
	text = text[:-2]
	return text
	...
	for k, v in dim_filters.items():
		if k == 'EmployeeName':
			df = df[(df[k].str.lower().isin([x.lower() for x in v])) | (
				df['EmployeeFirstName'].str.lower().isin([x.lower() for x in v])) | (
						df['EmployeeLastName'].str.lower().isin([x.lower() for x in v]))]
		else:
			df = df[df[k].astype(str).str.lower().isin([x.lower() for x in v])]
	return df
